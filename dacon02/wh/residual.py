...
class Residual(Layer):
    def __init__(self, kernel_size=3, filters=None, strides=1, padding='same', **kwargs):
        super(Residual, self).__init__(**kwargs)
        self.conv = Conv1D(kernel_size=kernel_size, filters=filters, strides=strides, padding=padding, kernel_initializer='he_uniform')
...
def build_cnn(input_shape, drop_rate=0.5, output_size=16):
    ...
    # Layer 2
    model.add(Residual(kernel_size=3, filters=128))
    ...
    # Layer 5
    model.add(Residual(kernel_size=3, filters=256))
    model.add(MaxPooling1D(pool_size=3, strides=3))
